...
var startQuaternion = q.identity(q.create());
var endQuaternion = q.identity(q.create());
var slerpQuaternion = q.identity(q.create());
...
(function(){
        ...
        var time = Math.abs(Math.sin(cnt/50));
        startQuaternion = q.rotate(rad,[1,0,0]);
        endQuaternion = q.rotate(rad,[0,1,0]);
        slerpQuaternion = q.slerp(startQuaternion,endQuaternion,time);

        ambientColor = [0.5, 0.0, 0.0, 1.0];
		draw(startQuaternion);
		ambientColor = [0.0, 0.5, 0.0, 1.0];
		draw(endQuaternion);
		ambientColor = [0.0, 0.0, 0.5, 1.0];
		draw(slerpQuaternion);

        function draw(qta:any){
            qMatrix = q.ToMat4x4(qta);
            mMatrix =m.identity(mMatrix);
            mMatrix =m.multiply(mMatrix,qMatrix);
            mMatrix =m.translate(mMatrix,[0,0,-5]);
